...
function Admin1Dashboard() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/auth/getleaves1")
      .then((response) => {
  ...
  const approveleave = (EmpLeaveID) => {
    axios
      .post(`http://localhost:5000/auth/approveleave2/${EmpLeaveID}`)
      .then((response) => {
        if (response.data.Status) {
  ...
  const disapproveleave = (EmpLeaveID) => {
    axios
      .post(`http://localhost:5000/auth/disapproveleave2/${EmpLeaveID}`)
      .then((response) => {
        if (response.data.Status) {
          const updatedEmployeeData = employeeData.map((employee) =>
            employee.EmpLeaveID === EmpLeaveID
      ...
      .catch((error) => {
        console.error("Error while disapproving leave:", error);
      });
  };
  ...
  const columns = [
    ...
    { title: "Start Date", dataIndex: "start_date", key: "start_date", width: "190px", render: formatDate, align: "center", },
    { title: "End Date", dataIndex: "end_date", key: "end_date", width: "120px", render: formatDate, align: "center", },
    {
      title: "Actions",
      ...
      render: (EmpLeaveID) => (
        <Space>
          <Button type="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-auto me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" onClick={() => approveleave(EmpLeaveID)}>
            Approve
          </Button>
          <Button type="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-auto me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" onClick={() => disapproveleave(EmpLeaveID)}>
            Disapprove
          </Button>
        </Space>
      ),
        ...
        <div className="row">
          <div className="col">
            <div id="mainheader1" className="p-2 flex flex-col items-center justify-center shadow">
              ...
              <h4 className="font-semibold text-3xl">Admin1 Dashboard</h4>
            </div>
