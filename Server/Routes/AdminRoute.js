...
router.get("/holiday", verifyToken, (req, res) => {
...
router.put("/edit_holi/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  const sql = `UPDATE holidays
        set occasion = ?, date = ?
        Where id = ?`;
  const values = [req.body.occasion, req.body.date];
...
router.post("/employee", verifyToken, (req, res) => {
  ...
  image.mv(`Public/Images/${fileName}`, (err) => {
    ...
    bcrypt.hash(req.body.password, 10, (err, hash) => {
      ...
      // Check jobtype and set leave values accordingly
      if (req.body.jobtype === "intern") {
        sickLeaveDays = 6;
        paidLeaveDays = 4;
      } else if (req.body.jobtype === "full_time") {
        sickLeaveDays = 12;
        paidLeaveDays = 12;
      }
...
router.put("/edit_employee/:id", verifyToken, (req, res) => {
  ...
  const sql = `UPDATE employee 
        set name = ?, mobile_no = ?, personal_email = ?, goloka_email = ?, salary = ?, address = ?, categories = ?, jobtype = ?, shift_timing = ?, employee_designation = ?
        Where id = ?`;
  const values = [
    req.body.name,
    req.body.mobile_no,
...
router.delete("/delete_category/:id", verifyToken, (req, res) => {
  const id = req.params.id;
  console.log(id);
...
router.post("/add_holi", verifyToken, (req, res) => {
  const sql = "INSERT INTO holidays (occasion, date) VALUES (?, ?)";
  console.log(req.body.occasion);
  con.query(sql, [req.body.occasion, req.body.date], (err, result) => {
    if (err) return res.json({ Status: false, Error: "Query Error" });
    return res.json({ Status: true });
  });
});

router.delete("/delete_holi/:id", verifyToken, (req, res) => {
  const id = req.params.id;
...
router.post("/sendOTP", verifyToken, async (req, res) => {
  ...
  const mailOptions = {
    ...
    text:
      "Use this " +
      otp +
      " One Time Password(OTP) to change your password.This is confidential please DO NOT SHARE YOUR OTP with anyone",
  };
  try {
    await transporter.sendMail(mailOptions);
    // alert("Please check your Email to get your OTP");
    return res.json({ Status: true, email, otp });
  } catch (error) {
    console.error("Error sending email:", error);
    return res.json({ Status: false, Error: "Failed to send email" });
  }
...
router.put("/newpass/:email", verifyToken, (req, res) => {
  console.log("Request Body", req.body);

  const sql = `UPDATE employee SET password=? WHERE goloka_email=?`;
