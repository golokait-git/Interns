  ...
  con.query(sql, values, (err, result) => {
    ...
    con.query(getAdmin2, async (err, res) => {
      if (err) {
        return res.json({
          Status: false,
          Error: "Failed to fetch Admin1 email",
        });
      }
      ...
      const mailOptions = {
        ...
        text:
          `This email is sent by your appplication.To let you know that a Sick leave has been 
        taken by: ` +
          employeeName +
          ` with Employee ID: ` +
          employeeID +
  ...
  con.query(sql, values, (err, result) => {
    ...
    con.query(getAdmin2, async (err, res) => {
      ...
      const mailOptions = {
        ...
        text:
          `This email is sent by your appplication.To let you know that a 
        Half leave has been taken by: ` +
          employeeName +
          `with Employee ID: ` +
          employeeID +
          ` For the day: ` +
...
EmployeeRouter.get("/holidays", verifyToken, (req, res) => {
